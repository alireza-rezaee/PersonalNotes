...
namespace AlirezaRezaee.PersonalNotes.WeblogApp.Data.Migrations.ApplicationDbContextMigrations
{
    ...
    {
        private static string RoleRecordMaker(string roleId, string roleName) => $"(N'{roleId}', N'{roleName}', N'{roleName.ToUpper()}', N'{Guid.NewGuid()}')\n";

        private static string RoleAssignRecordMaker(string roleId) => $"(N'c3ad1d11-31fc-4079-94d4-e74ab1273112', N'{roleId}')\n";

        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var roleQueryBuilder = new StringBuilder("INSERT INTO [AspNetRoles]([Id], [Name], [NormalizedName], [ConcurrencyStamp]) VALUES\n");
            var userRolesQueryBuilder = new StringBuilder("INSERT INTO [AspNetUserRoles] ([UserId], [RoleId]) VALUES\n");
            ...
            {
                var roleId = index.ToGuid().ToString();
                if (index != roles.Length - 1)
                {
                    roleQueryBuilder.Append(RoleRecordMaker(roleId, roles[index]) + ",\n");
                    userRolesQueryBuilder.Append(RoleAssignRecordMaker(roleId) + ",\n");
                }
                else
                {
                    roleQueryBuilder.Append(RoleRecordMaker(roleId, roles[index]));
            ...
            migrationBuilder.Sql(roleQueryBuilder.ToString());
            migrationBuilder.Sql(userRolesQueryBuilder.ToString());
        }
